...
async function sendAboutMentor(bot: TelegramBot, chatId: number) {
  await bot.sendMessage(chatId,
    `ℹ️ О менторе\n\n` +
    `👨‍🏫 Кирилл Анисимов — репетитор по физике с опытом более 5 лет.\n\n` +
    `🎓 Образование:\n` +
    `• Технический университет, физический факультет\n\n` +
    `🏆 Достижения учеников:\n` +
    `• Поступление в МФТИ, МГУ, МГТУ\n` +
    `• Победители районных и городских олимпиад\n` +
    `• Рост баллов ЕГЭ на 20-40 баллов\n\n` +
    `💡 Методика:\n` +
    `• Индивидуальный подход к каждому ученику\n` +
    `• Акцент на понимание, а не зубрёжку\n` +
...
async function sendFormatInfo(bot: TelegramBot, chatId: number) {
  await bot.sendMessage(chatId,
    ...
    `• Онлайн или офлайн\n` +
    `• Гибкое расписание\n` +
    `• Стоимость: уточняйте у ментора\n\n` +
    `👥 Групповые занятия:\n` +
    `• До 10 человек в группе\n` +
    `• Длительность: 90 минут\n` +
    `• Фиксированное расписание\n` +
    `• Стоимость ниже индивидуальных\n\n` +
    `💳 Абонементы:\n` +
    `• На 4 занятия — скидка 5%\n` +
    `• На 8 занятий — скидка 10%`,
    { reply_markup: MAIN_KEYBOARD }
  );
}
