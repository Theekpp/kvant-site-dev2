import { useGetBookings, useGetSubscriptions, useMarkBookingPaid, useMarkSubscriptionPaid } from "@/lib/hooks";
...
import { Banknote, CreditCard, CheckCircle2, XCircle } from "lucide-react";
...
export default function Payments() {
  ...
  const unpaidBookings = (bookings || []).filter((b: any) => !b.isPaid && b.status !== 'cancelled').length;
  const paidSubs = (subscriptions || []).filter((s: any) => s.isPaid).length;
  const unpaidSubs = (subscriptions || []).filter((s: any) => !s.isPaid).length;
              ...
              <TableBody>
                {sortedBookings.length === 0 ? (
                  ...
                  <TableRow key={b.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell>
                      <Badge variant="outline" className={`text-xs whitespace-nowrap ${
                        b.isPaid ? 'bg-green-100 text-green-800 border-green-200' : 'bg-gray-100 text-gray-500 border-gray-200'
                      }`}>
                        {b.isPaid ? '✓ Опл.' : '✗ Неопл.'}
                      </Badge>
                    </TableCell>
              ...
              <TableBody>
                {sortedSubs.length === 0 ? (
                  ...
                  <TableRow key={s.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell className="hidden sm:table-cell">
                      <Badge variant="outline" className="text-xs whitespace-nowrap">
                        {s.type === 'individual' ? 'Индивидуальный' : 'Групповой'}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge variant="outline" className={`text-xs whitespace-nowrap ${
                        s.isPaid ? 'bg-green-100 text-green-800 border-green-200' : 'bg-amber-100 text-amber-800 border-amber-200'
                      }`}>
                        {s.isPaid ? '✓ Актив.' : '⏳ Ожид.'}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      ...
                      {s.isPaid && <span className="text-xs text-muted-foreground">✓</span>}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
