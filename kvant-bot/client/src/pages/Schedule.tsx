...
import { useGetSchedule, useCreateScheduleSlot, useDeleteScheduleSlot } from "@/lib/hooks";
...
export default function Schedule() {
  ...
  const computeSlotsForDate = (date: Date, currentSlotType: "individual" | "group") => {
    ...
    const allForDay = (schedule || []).filter((s: any) =>
    ...
    const specificTimes = new Set(allForDay.filter((s: any) => s.specificDate === dateStr).map((s: any) => s.time));
    const deduped = allForDay.filter((s: any) => s.specificDate === dateStr || !specificTimes.has(s.time));
    ...
    return {
      primaryType,
      existingSlots: deduped.map((s: any) => ({ id: s.id, time: s.time })),
  ...
  const handleSubmit = async () => {
    ...
    if (draft.mode === "specific" && draft.date) {
      ...
      for (const time of draft.times) {
        // Skip if any slot already exists for this day+time (recurring)
        const alreadyExists = (schedule || []).some((s: any) =>
          s.time === time &&
          s.dayOfWeek === jsDay &&
          !s.specificDate
        );
  ...
  const allRecurring = (schedule || []).filter((s: any) => !s.specificDate);
              ...
              <>
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {Object.entries(recurringByDay).sort(([a], [b]) => Number(a) - Number(b)).flatMap(([dow, slots]) =>
                        slots.sort((a: any, b: any) => a.time.localeCompare(b.time)).map((slot: any) => (
                ...
                {specificCount > 0 && (
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {(schedule || []).filter((s: any) => s.specificDate)
                        .sort((a: any, b: any) => (a.specificDate || '').localeCompare(b.specificDate || '') || a.time.localeCompare(b.time))
                        .map((slot: any) => (
